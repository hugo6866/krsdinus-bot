            ...
            for (const course of krs) {
                ...
                if (idKelas) {
                    ...
                    if (input) {
                        console.log(input);
                         if (input.data != null) {
                             if (input.data === "Data Tersimpan") {
                                totalSuccess++;
                                if (totalSuccess === krs.length) break;
                            }
                        }
                    } else {
                        console.log(`${colors.red}${course.nama} - ${course.kodeMk} (${course.kelompok}) Gagal Input! at ${colors.green} ${new Date().toLocaleString()}...${colors.reset}`);
                    }
                } else {
                    console.log(`${colors.red}${course.nama} - ${course.kodeMk} (${course.kelompok}) Gagal! (Bentrok Matkul/Kuota SKS Habis/Kelas Penuh) at ${colors.green} ${new Date().toLocaleString()}...${colors.reset}`);
                }
